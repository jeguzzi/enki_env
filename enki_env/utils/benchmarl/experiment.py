...
if TYPE_CHECKING:
    ...
    from benchmarl.experiment import Callback, ExperimentConfig
...
class EnkiExperiment(Experiment):  # type: ignore[misc]
    """
    ...
    def __init__(
        ...
        env: ParallelEnkiEnv | None = None,
        eval_env: ParallelEnkiEnv | None = None,
        task: EnkiTaskClass | None = None,
        ...
        if not task and env:
            task = EnkiTaskClass('enki', {}, env=env, eval_env=eval_env)
    ...
    def load_policies(self,
                      path: PathLike | None = None,
                      name: str = 'policy') -> dict[str, OnnxPolicy]:
        """
        ...
        """
        return {
            group: self.load_policy(name="best_policy", group=group)
            for group in self.group_map
        }
    ...
    def action_space(self, group: str) -> gym.Space[Any]:
        ...
        return cast('gym.Space[Any]', self.test_env._env.action_space(agents[0]))
    ...
    def observation_space(self, group: str) -> gym.Space[Any]:
        ...
        if not agents:
            raise ValueError(f"Unknown group {group}!")
        return cast('gym.Space[Any]', self.test_env._env.observation_space(agents[0]))
    ...
    def get_single_agent_policies(self) -> dict[str, SingleAgentPolicy]:
        """
    ...
    @staticmethod
    def reload_from_file(restore_file: str) -> EnkiExperiment:
        """
